        ...
        <div className="container my-5" style={{ maxWidth: 1000 }}>
            ...
            <Card className="mb-4 shadow-sm">
                <Card.Header className="d-flex justify-content-between align-items-center">
                    ...
                    <Button variant="outline-primary" size="sm" onClick={openProfileModal}>Edit</Button>
                </Card.Header>
                <Card.Body>
                    <Row>
                        <Col md={6}><strong>Name:</strong> {displayValue(profile.name)}</Col>
                        <Col md={6}><strong>Email:</strong> {displayValue(profile.email)}</Col>
                    </Row>
                    <Row className="mt-2">
                        <Col md={6}><strong>Phone:</strong> {displayValue(profile.phone)}</Col>
                        <Col md={6}><strong>GST Number:</strong> {displayValue(profile.gstNumber)}</Col>
                    </Row>
                </Card.Body>
            </Card>
            ...
            <Card className="mb-4 shadow-sm">
                <Card.Header className="d-flex justify-content-between align-items-center">
                    ...
                    <Button variant="success" size="sm" onClick={() => openUserModal()}>Create User</Button>
                </Card.Header>
                <Card.Body>
                    {users.length === 0 ? (
                                    ...
                                    <tr key={idx}>
                                        ...
                                        <td>
                                            {Object.entries(u.permissions).map(([mod, acts]) => (
                                                <div key={mod}><strong>{mod}:</strong> {acts.join(", ")}</div>
                                            ))}
                                        </td>
                                        <td>
                                            <Button size="sm" variant="warning" className="me-2"
                                                onClick={() => openUserModal(u, idx)}>Edit</Button>
                                            <Button size="sm" variant={u.status === "active" ? "outline-danger" : "outline-success"}
            ...
            <Modal show={showProfileModal} onHide={() => setShowProfileModal(false)} centered>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={saveProfile} disabled={loadingProfile}>
                        {loadingProfile ? <Spinner animation="border" size="sm" /> : "Save Changes"}
                    </Button>
                </Modal.Footer>
            </Modal>

            {/* User Modal */}
            <Modal show={showUserModal} onHide={() => setShowUserModal(false)} size="lg" centered scrollable>
                <Modal.Header closeButton>
                    <Modal.Title>{editingIndex !== null ? "Edit User" : "Create New User"}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
                        <Row>
                        ...
                        {Object.entries(ALL_MODULES).map(([module, actions]) => (
                            <div key={module} className="d-flex" style={{ justifyContent: 'space-between' }}>
                                ...
                                <div className="d-flex flex-wrap mb-2">
                                    {actions.map((action) => (
                                        <Form.Check
                                            ...
                                            id={`${module}-${action}`}
                                            label={action}
                                            checked={newUser.permissions[module as ModuleName]?.includes(action) || false}
                                            onChange={() => togglePermission(module as ModuleName, action)}
                                            className="me-3"
                                        />
                                    ))}
                                </div>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="success" onClick={createOrUpdateUser} disabled={creatingUser}>
                        {creatingUser ? <Spinner animation="border" size="sm" /> : editingIndex !== null ? "Save Changes" : "Create User"}
                    </Button>
                </Modal.Footer>
            </Modal>
        </div>
