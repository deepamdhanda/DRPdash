...
import { createTicket, getAllTickets, updateTicket, getTicketById } from '../../../APIs/user/ticket';
...
const SupportChatWidget = () => {
    ...
    const [step, setStep] = useState<'categories' | 'subcategories' | 'articles' | 'fallback'>('categories')
    ...
    const [selectedSubcategory, setSelectedSubcategory] = useState<Topic | null>(null)
    ...
    const handleSubmitTicket = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!subject || !description || !priority || !selectedCategory || !selectedSubcategory) {
    ...
    const handleReplySubmit = async (e: React.FormEvent) => {
        ...
        if (replyAttachment) {
            try {
                ...
                const fileName = `tickets/replies/${Date.now()}_${replyAttachment.name}`;
                attachmentUrl = await createAmazonS3(fileName, base64String);
            } catch (err) {
        ...
        <>
                ...
                >
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: "space-between", gap: '8px' }}>
                        <p style={{ margin: '0 0 4px 0', fontWeight: 'bold' }}>
                            <span style={{ fontSize: '24px' }}>👋</span> Hi {Cookies.get('username')}!
                        </p>
                        <button style={{ border: "none", background: "#fff", cursor: 'pointer' }} onClick={() => { setOrderly(false)}}>
                            X
                        </button>
                    </div>
                    <p style={{ margin: '0 0 4px 0' }}>
                        I’m <strong>Orderly</strong> 💁‍♀️ — your assistant for a smoother shipping journey!
                    </p>
                    {stats && stats.counts.some((s: any) => s.count === 0) ? (
                        <>
                            <p style={{ margin: '0 0 8px 0' }}>
                                Here's your <strong>To-Do List</strong> — you're off to a great start 🚀
                            </p>
                            <ul style={{ paddingLeft: '18px', margin: 0, listStyle: 'none' }}>
                                {stats.counts.map((i: any) => (
                                    <li
                                        key={i.label}
                                        style={{ textDecoration: i.count > 0 ? "line-through" : "none" }}
                                    >
                        ...
                        <>
                            ...
                            {gyaan?.gyaan && (
                                ...
                                >
                                    <div style={{ position: "absolute", top: "-12px", right: "16px", fontSize: "1.5rem" }}>
                                        🤓🧠🔥
                                    </div>
                                    <h3 style={{ fontWeight: "bold", marginBottom: "10px", fontSize: "1.4rem" }}>
                                        🧠 Aaj Ka Dhamakedaar Gyaan!
                                    </h3>
                                    <p style={{ fontSize: "1.05rem", fontStyle: "italic", lineHeight: "1.6" }}>
                                        “{gyaan?.gyaan}”
                                    </p>
                                    <p style={{ marginTop: "12px", fontSize: "0.9rem", color: "#000" }}>
                                        💬 Gyaan accha laga? Toh like thoko, share karo aur doston ko bhi enlighten karo! 🚀
                                    </p>
                                </div>
                            )}
            ...
            {open && (
                ...
                >
                    ...
                    {/* Tab Content */}
                    <div style={{ padding: 12, overflowY: "auto", maxHeight: "calc(80vh - 100px)" }}>
                        {/* Tickets Tab */}
                        {tab === "tickets" && (
                            <>
                                {replyTicketId && replyTicketData ? (
                                    <div>
                                        ...
                                        <div className="mb-2 text-muted">
                                            <strong>Category:</strong> {replyTicketData.category?.name} - {replyTicketData.subcategory?.name}
                                        </div>
                                        <div className="mb-2">
                                            <strong>Status:</strong>{" "}
                                            <span className={replyTicketData.status === 'open' ? 'text-success' : 'text-secondary'}>
                                                {replyTicketData.status}
                                            </span>
                                        </div>
                                        <div className="mb-2">
                                            <strong>Description:</strong> {replyTicketData.description}
                                        </div>
                                        <div className="mb-2">
                                            <strong>Created:</strong> {new Date(replyTicketData.createdAt).toLocaleString()}
                                        </div>
                                        {replyTicketData.attachments && replyTicketData.attachments.length > 0 && (
                                            <div className="mb-2">
                                                <strong>Attachments:</strong>
                                                <ul>
                                                    {replyTicketData.attachments.map((url, idx) => (
                                                        <li key={idx} onClick={() => { setDisplayImage(url) }}><img src={url} style={{ width: "50px" }} /></li>
                                                    ))}
                                                </ul>
                                            </div>
                                        )}
                                        ...
                                        <div>
                                            ...
                                            <div style={{ maxHeight: 120, overflowY: "auto", marginBottom: 8 }}>
                                                {replyTicketData.replies && replyTicketData.replies.length > 0 ? (
                                                    replyTicketData.replies.map((rep, idx) => (
                                                        <div key={idx} style={{ marginBottom: 10 }}>
                                                            <div>
                                                                <span style={{ fontWeight: 500 }}>{rep.user?.name || "Support"}</span>
                                                            ...
                                                            {rep.attachments && rep.attachments.length > 0 && (
                                                                <div>
                                                                    {rep.attachments.map((url, i) => (
                                                                        <a key={i} href={url} target="_blank" rel="noopener noreferrer">Attachment {i + 1}</a>
                                                                    ))}
                                                                </div>
                                                            )}
                                                        </div>
                                            ...
                                            <Form onSubmit={handleReplySubmit}>
                                                ...
                                                <Form.Group className="mb-2">
                                                    ...
                                                    <Form.Control
                                                        type="file"
                                                        onChange={(e: any) => setReplyAttachment(e.target.files?.[0] || null)}
                                                        disabled={replyLoading}
                                                    />
                                                </Form.Group>
                                                <Button type="submit" variant="primary" size="sm" disabled={replyLoading}>
                                                    {replyLoading ? <Spinner size="sm" animation="border" /> : "Send Reply"}
                                                </Button>
                                            </Form>
                                        </div>
                                    </div>
                                    ...
                                    <Form onSubmit={handleSubmitTicket}>
                                        <p>
                                            ...
                                            <span className="text-primary fw-semibold">{selectedCategory?.name || "N/A"}</span>
                                            <span className="text-muted"> → </span>
                                            <span className="text-success fw-semibold">{selectedSubcategory?.name || "N/A"}</span>
                                        </p>
                                        <Form.Group className="mb-3">
                                            <Form.Label>Subject</Form.Label>
                                            <Form.Control name="subject" type="text" placeholder="Briefly summarize your issue" required />
                                        </Form.Group>
                                        <Form.Group className="mb-3">
                                        ...
                                        <Button variant="secondary" onClick={() => setIsNew(false)} className="me-2">
                                            Cancel
                                        </Button>
                                    ...
                                    <div>
                                        <div className="d-flex justify-content-between align-items-center mb-2" style={{ minWidth: "260px", }}>
                                        ...
                                        {loadingTickets ? (
                                            ...
                                            <div className="text-center text-muted py-4">No tickets found.</div>
                                        ) : (
                                            <div style={{ maxHeight: 220, overflowY: "auto" }}>
                                                {tickets.map((ticket) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="small">
                                                            <span>Status: </span>
                                                            <span className={ticket.status === 'open' ? 'text-success' : 'text-secondary'}>
                                                                {ticket.status}
                                                            </span>
                                                        </div>
                                                    </div>
                        ...
                        {tab === "help" && (
                            <div>
                                {step !== 'categories' && (
                                    <button
                                        onClick={() => resetToStep(step === 'articles' ? 'subcategories' : 'categories')}
                                        style={{
                                            background: 'none',
                                            border: 'none',
                                            color: '#666',
                                ...
                                {step === 'categories' && (
                                    <>
                                        <p style={{ marginBottom: '12px' }}>What can I help you with today?</p>
                                        {categories.map(cat => (
                                            <button
                                                key={cat._id}
                                ...
                                {step === 'articles' && (
                                    <>
                                        <p style={{ marginBottom: '12px' }}>
                                            Articles under <strong>{selectedSubcategory?.name}</strong>:
                                        </p>
                                        {articles.length > 0 ? (
                                ...
                                {(step === 'fallback' || step === 'articles') && (
                                    <div>
                                        <p style={{ margin: '12px 0', fontSize: "12px", }}>{step === 'articles' ? "Still confused? No worries, let’s get you the help you need." : "No worries, we're here to help!"}</p>
                                        <Button
                                            onClick={() => {
                                                setTab("tickets");
                                                setIsNew(true);
                    ...
                    <Modal show={!!displayImage} onHide={() => setDisplayImage(null)} >
                        <Modal.Body>
                            {displayImage && (<img src={displayImage} style={{ width: "100%", maxWidth: "600px" }} />)}
                        </Modal.Body>
                    </Modal>
                </div>
            )}
