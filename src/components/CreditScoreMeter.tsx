...
type Props = { score: number; label?: boolean; width?: number, validTill?: string | null };
...
const CreditScoreMeter: React.FC<Props> = ({ score, label = false, width = 120, validTill = null }) => {
  ...
  const angle = startAngle + ((displayScore - minVisible) / (maxVisible - minVisible)) * (endAngle - startAngle);
  ...
  const tooltip = `${raw}/900 — ${raw < 450 ? "High Risk" : raw < 650 ? "Moderate" : "Low Risk"}\nValid Until ${validTill || "N/A"}`;
    ...
    >
      <svg width={size} height={size*0.75} viewBox={`0 0 ${ size } ${ size*0.75 } `}>
        <defs>
          <linearGradient id="gloom" x1="0%" x2="0%" y1="0%" y2="100%">
            ...
            <stop offset="100%" stopColor="rgba(0,0,0,0.02)" stopOpacity="0.02" />
          </linearGradient>
        </defs>
        ...
        {zones.map((z, idx) => {
          const fromAngle = startAngle + ((z.from - minVisible) / (maxVisible - minVisible)) * (endAngle - startAngle);
          const toAngle = startAngle + ((z.to - minVisible) / (maxVisible - minVisible)) * (endAngle - startAngle);
          return (
            <path
              key={idx}
        ...
        >
          <line
            ...
            strokeLinecap="round"
          />
          <rect x={cx - 1} y={cy - needleLen} width={2} height={needleLen} fill="#222" opacity={0.95} />
          <circle cx={cx} cy={cy - needleLen + 8} r={6} fill="#fff" opacity={0.06} />
        </g>

        {/* center cap */}
        <circle cx={cx} cy={cy} r={6} fill="#fff" stroke="#0b1226" strokeWidth={1} />

        {/* subtle center label */}
        <text x={cx} y={cy + radius * 0.55} textAnchor="middle" fontSize={16} fill="#000434" fontWeight={800}>
          {raw}
        </text>
      ...
      {label && (
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 12, color: "#111", fontWeight: 800 }}>Ecom Credit Score</div>
          <div style={{ fontSize: 12, marginTop: 6, display: "flex", gap: 12, justifyContent: "center" }}>
